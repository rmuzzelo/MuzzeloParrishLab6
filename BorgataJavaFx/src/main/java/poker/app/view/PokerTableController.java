...
public class PokerTableController {
	...
	private void handlePlay() {
		...
		DealFaceDownCards(gme.getGameRuleDomainModel().getCOMMUNITYCARDSMAX(), 0);
	...
	private void handleDraw() {
		...
		if (act.isbEvaluateHand()) {
			...
			if (iCardDrawnPlayer + iCardDrawnCommon + 2 >= gme.getGameRuleDomainModel().getTotalCardsToDraw()) {
				for (Player p : mainApp.GetSeatedPlayers()) {
					Hand hPlayer = gme.FindPlayerGame(gme, p).getHand();
					for (int a = hPlayer.getCards().size(); a < gme.getGameRuleDomainModel().getPLAYERNUMBEROFCARDS(); a++) {
					...
					if (hCommon.getCards() == null) {
						for (int a = 0; a < gme.getGameRuleDomainModel().getCOMMUNITYCARDSMAX(); a++) {
							hCommon.AddCardToHand(new Card(eSuit.JOKER, eRank.JOKER, 0));
						}
					} else {

						for (int a = hCommon.getCards().size(); a < gme.getGameRuleDomainModel().getCOMMUNITYCARDSMAX(); a++) {
							hCommon.AddCardToHand(new Card(eSuit.JOKER, eRank.JOKER, 0));
						}
					}
