...
public class Action {
	...
	public Action(GamePlay gme, int iCardDrawnPlayer, int iCardDrawnCommon, int iDrawCount) {
		...
		if (iCardDrawnPlayer < gme.getGameRuleDomainModel().getPLAYERNUMBEROFCARDS()) {
			eDrawAction = eDrawAction.DrawPlayer;
			iCardDrawnPlayer++;
		} else if ((iCardDrawnPlayer >= gme.getGameRuleDomainModel().getPLAYERNUMBEROFCARDS())
				&& (iCardDrawnCommon <= gme.getGameRuleDomainModel().getCOMMUNITYCARDSMAX())) {
			eDrawAction = eDrawAction.DrawCommon;
			iCardDrawnCommon++;
		}
		if (iCardDrawnPlayer + iCardDrawnCommon == gme.getGameRuleDomainModel().getTotalCardsToDraw())
		{
			this.bEvaluateHand = true;
		}
		this.setiCardDrawn(gme.getiCardsToDraw(gme.getGameRuleDomainModel()).get(iDrawCount-1));
	}
